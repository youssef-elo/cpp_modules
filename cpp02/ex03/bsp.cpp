...
bool bsp( Point const a, Point const b, Point const c, Point const point)
{
	...
	std::cout << surface1 << std::endl << surface2 << std::endl << surface3 << std::endl << "triangle " << triangle_surface << std::endl;
	...
	if ( surface1 > zero && surface2 > zero && surface3 > zero && total_surface <= triangle_surface)
		return true;
	return false;
}